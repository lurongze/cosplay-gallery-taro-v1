...
class Index extends Taro.Component {
  ...
  render () {
    const { indexStore: { list, loadingList,likeMost }, commonStore: { defaultAvatar } } = this.props
    return (
              ...
              return (
                <View className='list-item' key={item.id}>
                  <View className='list-header'>
                    <View className='list-avatar' style={{backgroundImage: `url(https://icon-1253286615.cos.ap-guangzhou.myqcloud.com/avatar-cg.png)`}} />
                    <View className='list-title'>
                      {/*<View className='list-user'>{item.authorName || '匿名用户'}</View>*/}
                      <View className='list-user'>#{item.category}#</View>
